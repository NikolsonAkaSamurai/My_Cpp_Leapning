...
class IOContours {
private:
    inline static const std::string CONTOURS_DIR = "data/contours";
...
public:
    ...
    static Contour readContour(const std::string& name) {
        ...
        if (!file) return result;  // ← если файл не открылся, возвращаем пустой контур
    ...
    static std::vector<std::string> getAvailableContours() {
        ...
        return contours;  // ← всегда возвращаем (даже пустой вектор)
    }
};